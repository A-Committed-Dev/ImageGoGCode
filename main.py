...
def black_white(img, cIndex):
    ...
    return index[0] + "_black_white" + ".png"
...
def get_img_cordinates(img):
    ...
    for x in range(width):
        for y in range(height):
            ...
            elif image.getpixel((x,
                                 y)) == 255:  # checks if pixel is white and adds templist to vectorlist if templist is bigger than 0
                if len(templist) > 0:
...
create_gcode(get_real_cordinates(get_img_cordinates(black_white(resize_img("duck.svg"), 150))))
